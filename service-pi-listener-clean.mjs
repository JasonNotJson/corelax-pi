...
const publishString = (topic, str) =>
  new Promise((res, rej) =>
    mqttClient.publish(topic, String(str), { qos: 1, retain: false }, (err) =>
      err ? rej(err) : res()
    )
  );
  ...
  try {
    ...
    if (type === "open_door") {
      ...
      await Promise.allSettled([
        publishJson(canon, msg).then(() => {
          publishedOk = true;
        }),
        publishJson(doorAlias(), msg).then(() => {
          publishedOk = true;
        }),
      ]);
      ...
      await Promise.allSettled([
        ...
        publishJson(waterAlias(), msg).then(() => {
          publishedOk = true;
        }),
        publishString(waterAlias(), "START")
          .then(() => {
            publishedOk = true;
          })
          .catch(() => {}),
      ...
      type === "fan_start" ||
      (type === "chair_start" && isFanTarget(target))
    ) {
      const tgt = target || "esp-fan-01";
      const canon = canonicalTopic(tgt);
      await Promise.allSettled([
        publishString(fanAlias(), "START").then(() => {
          publishedOk = true;
        }),
        publishJson(canon, { id, action: "START", args: {} })
          .then(() => {
            publishedOk = true;
          })
          .catch(() => {}),
      ...
      const canon = canonicalTopic(tgt);
      await Promise.allSettled([
        publishString(fanAlias(), "STOP").then(() => {
          publishedOk = true;
        }),
        publishJson(canon, { id, action: "STOP", args: {} })
          .then(() => {
            publishedOk = true;
          })
          .catch(() => {}),
      ...
      const minutes = Math.max(
        1,
        p.minutes ?? p.chairMinutes ?? p.args?.minutes ?? 15
      );
      ...
      await Promise.allSettled([
        ...
        alias
          ? publishString(alias, "START").then(() => {
              publishedOk = true;
            })
          : Promise.resolve(),
      ]);
      ...
      throw new Error(
        "unknown/unsupported command_type: " + (cmd?.command_type || "")
      );
    }
...
async function pollPending() {
  try {
    const { data: rows, error } = await supabase.rpc("pending_for_device", {
      p_device_id: ENV.DEVICE_ID,
    });
    if (error) {
